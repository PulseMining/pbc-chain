...
void NodeStats::add_log(const std::string& line, int color_type) {
    ...
    if (!log_lines.empty() && log_lines.back().first == line) {
        return;
    }
...
void play_startup_banner() {
    fprintf(stderr, "\033[2J\033[H");
    fprintf(stderr, "\n");
    fprintf(stderr, "\033[36m  ╔══════════════════════════════════════╗\033[0m\n");
    fprintf(stderr, "\033[36m  ║   PRIVACY BANK CHAIN — Node v1.0    ║\033[0m\n");
    fprintf(stderr, "\033[36m  ║   rx/pbc — CPU Mining               ║\033[0m\n");
    fprintf(stderr, "\033[36m  ╚══════════════════════════════════════╝\033[0m\n");
    fprintf(stderr, "\n");
    fprintf(stderr, "\033[33m  Starting PBC daemon...\033[0m\n\n");
    fflush(stderr);
    usleep(1000000);
}
...
static void init_colors() {
    start_color();
    if (can_change_color()) {
        init_color(20, 260, 220, 560);    // PBC dark blue bg
        init_color(21, 420, 370, 710);    // PBC light blue
    ...
    while (!stop_signal.load()) {
        // Poll RPC for peer count every 5 seconds
        {
            ...
            if (std::chrono::duration_cast<std::chrono::seconds>(now_poll - last_rpc_poll).count() >= 5) {
                ...
                FILE* rpc = popen("curl -s http://127.0.0.1:18831/get_info 2>/dev/null", "r");
                if (rpc) {
                    ...
                    try {
                        ...
                        uint64_t rpc_height = fv("height");
                        uint64_t rpc_target = fv("target_height");
                        if (rpc_height > 0) stats.height.store(rpc_height);
                        if (rpc_target > 0) stats.target_height.store(rpc_target);
                        if (rpc_target == 0 || rpc_height >= rpc_target) stats.synced.store(true);
                        else stats.synced.store(false);
                    } catch (...) {}
                }
            }
        }
        ...
        // Header
        draw_centered(y++, w, "**** PBC CHAIN NODE V1.0 ****", CP_HEADER);
        y++;
        ...
        print_stat(0, "STATUS:", tmp, (status_str == "MINING" || status_str == "SYNCED") ? CP_SUCCESS : CP_NORMAL);
        ...
        snprintf(tmp, sizeof(tmp), "%s PBC", reward_str.c_str());
        ...
        // Footer
        draw_centered(h - 2, w, "CTRL+C TO QUIT  -  PBC CHAIN (C) 2026", CP_HEADER);
...
void tui_log_handler(const std::string& message) {
    ...
    // PBC: Filter C64 legacy and irrelevant daemon messages
    if (message.find("c64wallet") != std::string::npos) return;
    if (message.find("You may now start") != std::string::npos) return;
    if (message.find("Use \"help\"") != std::string::npos) return;
    if (message.find("Use the \"help\"") != std::string::npos) return;
    if (message.find("64K RAM") != std::string::npos) return;
    if (message.find("38911 BASIC") != std::string::npos) return;
    if (message.find("READY.") != std::string::npos && message.find("HEIGHT") == std::string::npos) return;
    ...
    if (message.find("error") != std::string::npos || message.find("ERROR") != std::string::npos)
        color = LOG_ERROR;
    else if (message.find("initialized OK") != std::string::npos || message.find("SYNCED") != std::string::npos)
        color = LOG_SUCCESS;
...
void PBCTuiLogCallback::handle(const el::LogDispatchData* data) {
    try {
...
void install_tui_log_callback() {
    el::Helpers::installLogDispatchCallback<PBCTuiLogCallback>("PBCTuiCallback");
}
